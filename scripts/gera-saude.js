...
const FONTES={precos:['Preços',80,72],bdrs:['Catálogo BDR',100,null],etfs:['Catálogo ETF',100,null],etfsDetalhes:['Detalhamento ETF',null,null],metricasBdr:['Histórico BDR',90,96],metricasEmpresas:['Histórico empresas',75,96],analise:['Ativo lastro',70,96],eventos:['Documentos CVM',null,120]};
const ARQUIVOS={precos:'precos.json',bdrs:'bdrs.json',etfs:'etfs.json',etfsDetalhes:'etfs-detalhes.json',metricasBdr:'metricas.json',metricasEmpresas:'metricas-empresas.json',analise:'analise.json',eventos:'eventos.json'};
...
function avaliaFonte(x,agora,chave=''){
  ...
  const disponivel=['bdrs','etfs'].includes(chave),registros=disponivel?x.registros:x.renovados;
...
function gera(bases,agora=Date.now()){
  ...
  for(const [k,[nome,minCobertura,limiteHoras]] of Object.entries(FONTES)){
    ...
    const preservados=validos.filter(v=>v.stale===true||(k==='etfs'&&v.tickerVerificado===false)).length,renovados=validos.length-preservados;
    const datas=validos.map(v=>k==='precos'&&Number.isFinite(v.t)&&v.t>0&&v.t<8640000000000?new Date(v.t*1000).toISOString().slice(0,10):k==='analise'?v.dtAtivo:k==='etfsDetalhes'?v.revisadoEm:v.dt).filter(data).sort();
